...
class GTADistillationLoss:
    ...
    def _generate_masks(self, batch, teacher_preds=None):
        ...
        Excludes regions where teacher model detects incorrectly (False Positives and False Negatives).
        Distills the remaining regions where teacher is likely correct (True Positives and Background).
        """
    ...
    def _bbox_iou_batch(self, boxes1, boxes2):
        """
        boxes1: (K, 4) xywh normalized or pixels
        boxes2: (M, 4) xywh normalized or pixels
        returns: (K, M)
        """
        # Convert to xyxy
        b1_x1, b1_y1, b1_x2, b1_y2 = boxes1[:, 0] - boxes1[:, 2] / 2, boxes1[:, 1] - boxes1[:, 3] / 2, \
                                     boxes1[:, 0] + boxes1[:, 2] / 2, boxes1[:, 1] + boxes1[:, 3] / 2
        b2_x1, b2_y1, b2_x2, b2_y2 = boxes2[:, 0] - boxes2[:, 2] / 2, boxes2[:, 1] - boxes2[:, 3] / 2, \
